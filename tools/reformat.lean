...
Patched for newer Lean 4 API around InfoTree.context and CoreM.toIO.
-/
import Lean
...
def parseModule
  ...
  let topLevelCmds : Array CommandSyntax ←
    s.commandState.infoState.trees.toArray.mapM fun
      | InfoTree.context (PartialContextInfo.commandCtx ctx)
          (InfoTree.node (Info.ofCommandInfo cmdInfo) _) =>
        pure {
          env := ctx.env
...
unsafe def main (args : List String) : IO Unit := do
  ...
  let leadingUpdated :=
    mkNullNode (moduleStx.map (·.stx)) |>.updateLeading |>.getArgs
  ...
  for { env, currNamespace, openDecls, .. } in moduleStx, stx in leadingUpdated do
    ...
    let _ ← (printCommands stx).toIO coreCtx coreState