...
def writeSourceViewerHtml
    ...
    (outPath : String := "lean_source_viewer.html") : IO Unit := do
  ...
  let css :=
    ...
    ".trm{font-size:.68rem;color:var(--dm);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:24vw;}" ++
    ".trm span{color:var(--tx);background:var(--bd);padding:.1em .4em;border-radius:3px;}" ++

    ".ctrls{display:flex;align-items:center;gap:.5rem;font-size:.58rem;color:var(--dm);white-space:nowrap;}" ++
    ...
    ".main{display:grid;grid-template-columns:1.25fr .75fr;grid-template-rows:42vh 1fr;gap:1px;background:var(--bd);height:calc(100vh - 56px);overflow:hidden;}" ++
  ...
  let body :=
    "<header><h1>Lean Source Viewer</h1>" ++
    ...
    "<div class=\"ctrls\">" ++
      ...
      "<button class=\"btn\" id=\"corrBtn\">Corr</button>" ++
    "</div>" ++
    ...
    "<div class=\"main\">" ++
      ...
      "<div class=\"panel sourcePanel\">" ++
        "<div class=\"ph\"><div class=\"pn\">1</div><div class=\"pt\">Annotated Surface Source</div><div class=\"ps\">leaf tokens with metadata</div></div>" ++
        "<div class=\"pb\"><div class=\"sourceWrap\">" ++
          "<div class=\"sourceIntro\">Current term:<br><span class=\"termPreview\" id=\"termPreview\"></span><br>Surface Syntax の leaf node をソーストークン風に並べています。クリックすると右側に metadata を表示します。</div>" ++
      ...
      "<div class=\"panel graphPanel\">" ++
        "<div class=\"ph\"><div class=\"pn\">G</div><div class=\"pt\">Mini Graph Diff</div><div class=\"ps\">Surface ↔ Delaborated</div></div>" ++
        "<div class=\"pb\">" ++
          "<div id=\"cy\"></div>" ++
  ...
  let js :=
    ...
    "function short(s,n=28){return s.length>n?s.slice(0,n-2)+'…':s;}" ++
    ...
    "function collectLeaves(root,phase,pathKeysOther){" ++
      ...
      "function walk(n,path,recursivePath){" ++
        ...
        "if(isLeaf){" ++
          ...
          "out.push({label:labelOf(n),shortLabel:short(labelOf(n),24),phase:phase,type:n.type||'',kind:n.kind||'',val:n.val||'',path:path,recursivePath:recursivePath,key:key,occ:occ,status:status});" ++
        "}" ++
        "if(n.children)n.children.forEach((c,i)=>walk(c,path+'_'+i,k+'['+i+']'));" ++
      "}" ++
      "walk(root,'0','');" ++
      "return out;" ++
    "}" ++
    ...
    "const delabByKey={};" ++
    "const delabByKeyOcc={};" ++
    "delabLeaves.forEach(t=>{" ++
      "if(!delabByKey[t.key])delabByKey[t.key]=[];" ++
      "delabByKey[t.key].push(t);" ++
      "delabByKeyOcc[t.key+'#'+t.occ]=t;" ++
    "});" ++

    "function candidatesForToken(t){" ++
      "const exact=delabByKeyOcc[t.key+'#'+t.occ];" ++
      "if(exact) return [exact];" ++
      "return (delabByKey[t.key]||[]).slice(0,4);" ++
    "}" ++

    "function interpretationForToken(t,cands){" ++
      "if(t.status==='lost' && cands.length===0){" ++
        "return 'Surface にのみ現れる leaf です。Delaborated 側に同名候補が見つからないため、括弧・hygiene・記法上の包みなど、意味表現に残らない情報の可能性があります。';" ++
      "}" ++
      "if(t.status==='lost' && cands.length>0){" ++
        "return 'Delaborated 側に同じ label の候補はありますが、構造上の位置対応は一致していません。元の token が保存されたというより、delaboration により似た表示が再構成された可能性があります。';" ++
      "}" ++
      "if(t.status==='preserved' && cands.length>0){" ++
        "return 'Delaborated 側に同じ label の候補があります。leaf レベルでは保存または再出現しているように見えます。ただし、これは意味的対応の証明ではなく、label と occurrence に基づく候補です。';" ++
      "}" ++
      "if(t.status==='preserved' && cands.length===0){" ++
        "return 'Surface 側では preserved 扱いですが、候補抽出では Delaborated leaf が見つかっていません。対応規則をさらに改善できる余地があります。';" ++
      "}" ++
    ...
    "function renderSource(){" ++
      ...
      "surfaceLeaves.forEach(t=>{" ++
        ...
        "span.dataset.path=t.path;" ++
        "span.addEventListener('click',()=>selectToken(t));" ++
        "span.addEventListener('mouseover',e=>showTip(e,t.label,'status='+t.status+' / kind='+(t.kind||'')+' / type='+(t.type||'')+' / occ='+String(t.occ)));" ++
    ...
    "function metaHtml(t){" ++
      ...
      "h+='<div class=\"k\">occurrence</div><div class=\"v\">'+String(t.occ)+'</div></div></div>';" ++

      "h+='<div class=\"box\"><div class=\"boxTitle\">Delaborated candidates</div>';" ++
      "if(cands.length===0){" ++
        "h+='<div class=\"v\">No candidate found.</div>';" ++
      "}else{" ++
        "cands.forEach((c,i)=>{" ++
          "h+='<div class=\"kv\"><div class=\"k\">candidate '+(i+1)+'</div><div class=\"v\">'+escapeHtml(c.label)+' / '+escapeHtml(c.path)+' / occ='+String(c.occ)+'</div></div>';" ++
        "});" ++
      "}" ++
      "h+='</div>';" ++
    ...
    "function selectToken(t){" ++
      ...
      "const tok=document.querySelector('.tok[data-path=\"'+t.path+'\"]');" ++
      ...
      "if(window.cy){" ++
        ...
        "const node=cy.$('#s'+t.path);" ++
        "if(node.length){" ++
          "const connected=node.connectedEdges();" ++
          "node.addClass('selectedFocus');" ++
    ...
    "function flattenGraph(root,prefix,phase,baseX,pathKeysOther){" ++
      ...
      "function walk(n,depth,parentId,path,recursivePath){" ++
        ...
        "const label=short(labelOf(n),20);" ++
    ...
    "const sg=flattenGraph(D.tree1,'s','surface',80,dKeys);" ++
    "const dg=flattenGraph(D.tree3,'d','delab',680,sKeys);" ++

    "let corr=[];let corrN=0;" ++
    "Object.keys(sg.buckets).forEach(k=>{" ++
      "const a=sg.buckets[k]||[];const b=dg.buckets[k]||[];const m=Math.min(a.length,b.length);" ++
      "for(let i=0;i<m;i++){corr.push({data:{id:'corr_'+corrN,source:a[i],target:b[i],kind:'corr'}});corrN++;}" ++
    "});" ++

    "const removed=sg.nodes.filter(n=>n.data.diff==='removed').length;" ++
    "const added=dg.nodes.filter(n=>n.data.diff==='added').length;" ++
    ...
    "window.cy=cytoscape({" ++
      ...
      "elements:[...sg.nodes,...dg.nodes,...sg.edges,...dg.edges,...corr]," ++
      ...
      "style:[" ++
        ...
        "{selector:'edge[kind=\"child\"]',style:{'width':1,'line-color':'#1e1e2e','target-arrow-color':'#1e1e2e','target-arrow-shape':'triangle','curve-style':'bezier','opacity':0.78}}," ++
        "{selector:'edge[kind=\"corr\"]',style:{'width':1.4,'line-color':'#f9e2af','target-arrow-color':'#f9e2af','target-arrow-shape':'vee','curve-style':'bezier','line-style':'dashed','opacity':0.32}}," ++
    ...
    "cy.on('mouseover','node',evt=>{" ++
      ...
      "showTip(evt.originalEvent,d.fullLabel||d.label,'phase='+d.phase+' / diff='+d.diff+' / path='+d.path);" ++
    "});" ++
    ...
    "cy.on('tap','node',evt=>{" ++
      ...
      "}else{" ++
        "document.getElementById('metaPanel').innerHTML='<div class=\"box\"><div class=\"boxTitle\">Delaborated node</div><div class=\"kv\"><div class=\"k\">label</div><div class=\"v\">'+escapeHtml(d.fullLabel||d.label)+'</div><div class=\"k\">kind</div><div class=\"v\">'+escapeHtml(d.kind||'')+'</div><div class=\"k\">type</div><div class=\"v\">'+escapeHtml(d.type||'')+'</div><div class=\"k\">diff</div><div class=\"v\">'+escapeHtml(d.diff||'')+'</div><div class=\"k\">path</div><div class=\"v\">'+escapeHtml(d.path||'')+'</div></div><div class=\"box\"><div class=\"boxTitle\">Interpretation</div><div class=\"v\">このノードは Delaborated Syntax 側のノードです。Surface 側の token をクリックすると、対応候補と情報消失の説明を確認できます。</div></div></div>';" ++
      "}" ++
    "});" ++
    ...
    "document.getElementById('fitBtn').addEventListener('click',()=>cy.fit(undefined,45));" ++

    "let lostOnly=false;" ++
    "document.getElementById('lostBtn').addEventListener('click',()=>{" ++
      "lostOnly=!lostOnly;" ++
      "document.querySelectorAll('.tok').forEach(el=>{" ++
        "if(lostOnly && !el.classList.contains('lost'))el.classList.add('dim');else el.classList.remove('dim');" ++
      "});" ++
      "cy.elements().removeClass('faded');" ++
      "if(lostOnly){cy.nodes('[diff=\"same\"]').addClass('faded');cy.edges('[kind=\"child\"]').addClass('faded');}" ++
    "});" ++
  ...
  let html :=
    "<!DOCTYPE html><html lang=\"ja\"><head><meta charset=\"UTF-8\"><title>Lean Source Viewer</title>" ++
...
elab "#analyze_term " t:term : command => do
  Command.liftTermElabM do
    ...
    logInfo m!"✓ lean_source_viewer.html を生成しました。ブラウザで開いてください。"
