...
def sourceViewerBody : String :=
    ...
    "<div class=\"main sourceMode\" id=\"main\">" ++
      ...
      "<div class=\"panel graphPanel\">" ++
        "<div class=\"ph\"><div class=\"pn\">G</div><div class=\"pt\">Mini Graph Diff</div><div class=\"ps\">candidate score edges</div></div>" ++
        "<div class=\"pb\">" ++
          "<div id=\"cy\"></div>" ++
...
def sourceViewerJs : String :=
    ...
    "function pathDistance(a,b){" ++
      "const xs=String(a).split('_');" ++
      "const ys=String(b).split('_');" ++
    ...
    "function collectLeaves(root,phase,pathKeysOther){" ++
      ...
      "function walk(n,path,recursivePath,depth,parentKey){" ++
        ...
        "if(isLeaf){" ++
          ...
          "out.push({label:labelOf(n),shortLabel:short(labelOf(n),24),phase:phase,type:n.type||'',kind:n.kind||'',val:n.val||'',path:path,recursivePath:recursivePath,key:key,occ:occ,status:status,start:n.start,end:n.end,depth:depth,parentKey:parentKey||''});" ++
        "}" ++
    ...
    "function metaHtml(t){" ++
      ...
      "}else{" ++
        "cands.forEach((c,i)=>{" ++
          "h+='<div class=\"kv\"><div class=\"k\">candidate '+(i+1)+'</div><div class=\"v\">'+escapeHtml(c.label)+' / '+escapeHtml(c.path)+' / occ='+String(c.occ)+' / <span class=\"score\">score='+String(c.score)+'</span> / range='+escapeHtml(String(c.start))+'..'+escapeHtml(String(c.end))+'</div></div>';" ++
        "});" ++
      "}" ++
      "h+='</div>';" ++
    ...
    "function flattenGraph(root,prefix,phase,baseX,pathKeysOther){" ++
      ...
      "function walk(n,depth,parentId,path,recursivePath,parentKey){" ++
        ...
        "order++;" ++
        "nodes.push({data:{id:id,label:label,fullLabel:fullLabel,phase:phase,type:n.type||'',kind:n.kind||'',val:n.val||'',diff:diff,path:path,key:key,start:n.start,end:n.end,depth:depth,parentKey:parentKey||''},position:{x:x,y:y}});" ++
    ...
    "const dg=flattenGraph(D.tree3,'d','delab',700,sKeys);" ++

    "const delabNodeById={};" ++
    "dg.nodes.forEach(n=>delabNodeById[n.data.id]=n.data);" ++
    "let corr=[];let corrN=0;" ++
    "surfaceLeaves.forEach(s=>{" ++
      "const cands=candidatesForToken(s).slice(0,2);" ++
      "cands.forEach(c=>{" ++
        "const targetId='d'+c.path;" ++
        "if(delabNodeById[targetId]){" ++
          "corr.push({data:{id:'corr_'+corrN,source:'s'+s.path,target:targetId,kind:'corr',score:c.score,label:String(c.score)}});" ++
          "corrN++;" ++
        "}" ++
      "});" ++
    "});" ++
    ...
    "window.cy=cytoscape({" ++
      ...
      "style:[" ++
        ...
        "{selector:'edge[kind=\"corr\"]',style:{'width':1.4,'line-color':'#f9e2af','target-arrow-color':'#f9e2af','target-arrow-shape':'vee','curve-style':'bezier','line-style':'dashed','opacity':0.32,'label':'data(label)','font-size':'8px','color':'#f9e2af','text-background-color':'#0a0a0f','text-background-opacity':0.7}}," ++
    ...
    "cy.on('mouseover','node',evt=>{" ++
      ...
      "showTip(evt.originalEvent,d.fullLabel||d.label,'phase='+d.phase+' / diff='+d.diff+' / range='+d.start+'..'+d.end+' / path='+d.path);" ++
    "});" ++
    ...
    "function setMode(mode){" ++
      ...
      "const sourceBtn=document.getElementById('sourceModeBtn');" ++
      "const graphBtn=document.getElementById('graphModeBtn');" ++
      "if(mode==='graph'){" ++
        "main.classList.remove('sourceMode');main.classList.add('graphMode');" ++
        "sourceBtn.classList.remove('active');graphBtn.classList.add('active');" ++
      "}else{" ++
        "main.classList.remove('graphMode');main.classList.add('sourceMode');" ++
        "graphBtn.classList.remove('active');sourceBtn.classList.add('active');" ++
      "}" ++
      "setTimeout(()=>cy.fit(undefined,45),80);" ++
    "}" ++
